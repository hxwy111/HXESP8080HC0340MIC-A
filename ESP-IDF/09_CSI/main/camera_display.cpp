...
// 新硬件：R43 已删除、C39 改为 10kΩ 下拉；GPIO33 必须由软件主动控制。
...
esp_err_t camera_display_show(const void *pixels, size_t size) {
    ...
    // 一次复制本身约几十毫秒；每个小块都延时会把预览降到几帧/秒。
    // 保持连续 memcpy，完整帧复制结束后只延时一个 tick，让 IDLE0 喂狗。
    // 源帧仍由 acquire/release 保证不会被 DMA 覆盖。
    ...
    if (!lit) {
        vTaskDelay(pdMS_TO_TICKS(20)); // 先让首帧进入扫描再开启背光。
